package org.example.soa_lab2_LabService.ws.validation.validator;

import jakarta.validation.ConstraintValidator;
import jakarta.validation.ConstraintValidatorContext;
import org.apache.commons.lang3.math.NumberUtils;
import org.example.soa_lab2_LabService.common.util.ConstraintUtil;
...
public class LabDtoValidator implements ConstraintValidator<LabDtoConstraint, LabDto> {
    ...
    private boolean labDtoValidation(LabDto labDto, ConstraintValidatorContext context) {
        ...
        if (!(Objects.isNull(labDto.getName()) || labDto.getName().trim().equals("")) && labDto.getName().length() > 254)
            res = ConstraintUtil.addConstraintViolation(context, "Поле Name не может быть не может быть больше 254 символа");
    ...
    private boolean personDtoValidation(LabDto labDto, ConstraintValidatorContext context) {
        ...
        if (!(Objects.isNull(labDto.getAuthor().getName()) || labDto.getAuthor().getName().trim().equals("")) && labDto.getAuthor().getName().length() > 254)
            res = ConstraintUtil.addConstraintViolation(context, "Поле Author -> Name не может быть больше 254 символа");
